...
const views={dashboard:{title:"Dashboard"},presupuestos:{title:"Presupuestos"},clientes:{title:"Clientes"},pagos:{title:"Pagos"},finanzas:{title:"Finanzas"},proyectos:{title:"Proyectos",html:`<article class="card"><h3>Proyectos</h3><p class="muted">Seguimiento de fases, responsables, fechas y avance.</p></article>`},configuracion:{title:"Configuración",html:`<article class="card"><h3>Configuración y seguridad</h3><p class="muted">Usuarios, roles, empresa, IVA, tasa BCV, cuentas y parámetros del sistema.</p></article>`}};
async function render(name){const item=views[name]||views.dashboard;title.textContent=item.title;document.querySelectorAll(".nav-item").forEach(el=>el.classList.toggle("active",el.dataset.view===name));if(name==="dashboard"){view.innerHTML=`<article class="card loading-card"><p class="muted">Consultando información...</p></article>`;try{view.innerHTML=dashboardHTML(await getDashboardStats());}catch(error){console.error(error);view.innerHTML=`<article class="card"><h3>No se pudo cargar el Dashboard</h3><p class="muted">Firestore no respondió correctamente.</p></article>`;}return;}if(name==="clientes"){await renderClientes(view);return;}if(name==="presupuestos"){await renderPresupuestos(view);return;}if(name==="pagos"){await renderPagos(view);return;}if(name==="finanzas"){await renderFinanzas(view);return;}view.innerHTML=item.html;}
