...
const views = {
  ...
  pagos: { title: "Pagos", html: `<article class="card"><h3>Pagos</h3><p class="muted">Aquí aparecerán los comprobantes recibidos, la tasa del día y la verificación administrativa.</p></article>` },
...
async function render(name) {
  ...
  if (name === "dashboard") {
    ...
    try { view.innerHTML = dashboardHTML(await getDashboardStats()); }
    catch (error) { console.error(error); view.innerHTML = `<article class="card"><h3>No se pudo cargar el Dashboard</h3><p class="muted">Firestore no respondió correctamente.</p></article>`; }
    return;
  }
  if (name === "clientes") { await renderClientes(view); return; }
  if (name === "presupuestos") { await renderPresupuestos(view); return; }
...
onAuthStateChanged(auth, async currentUser => {
  ...
  try {
    ...
    badge.textContent = profile?.rol ? `${currentUser.email} · ${profile.rol}` : (currentUser.email || "Usuario");
