...
export async function renderClientes(container) {
  ...
  try {
    ...
    const bindTableActions = () => container.querySelectorAll("#clientTable [data-action]").forEach(button => button.addEventListener("click", async () => { const action = button.dataset.action; if (action === "new-client") openForm(); if (action === "edit") openForm(clientes.find(c => c.id === button.dataset.id)); if (action === "toggle") await toggleCliente(button.dataset.id); }));
    const render = async () => {
      ...
      search.addEventListener("input", () => { const term = search.value.trim().toLowerCase(); current = clientes.filter(c => [c.nombre, c.rif, c.contacto, c.telefono, c.whatsapp, c.email].some(value => String(value || "").toLowerCase().includes(term))); container.querySelector("#clientCount").textContent = `${current.length} cliente${current.length === 1 ? "" : "s"}`; container.querySelector("#clientTable").innerHTML = tableHTML(current); bindTableActions(); });
